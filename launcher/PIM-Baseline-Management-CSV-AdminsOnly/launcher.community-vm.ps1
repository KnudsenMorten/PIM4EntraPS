...
<#
.SYNOPSIS
    Community VM launcher for PIM4EntraPS\PIM-Baseline-Management-CSV-AdminsOnly. Invokes the engine
...
.DESCRIPTION
    Runs PIM4EntraPS\PIM-Baseline-Management-CSV-AdminsOnly on a Windows box in the customer's own tenant.
...
.NOTES
    ...
    Engine         : PIM-Baseline-Management-CSV-AdminsOnly
...
if (-not (Test-Path -LiteralPath $engine)) {
    throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\PIM-Baseline-Management-CSV-AdminsOnly\PIM-Baseline-Management-CSV-AdminsOnly.ps1."
}
& $engine
