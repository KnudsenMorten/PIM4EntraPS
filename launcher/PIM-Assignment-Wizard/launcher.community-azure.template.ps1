...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'PIM-Assignment-Wizard (community cloud) starting'
...
if (-not $engine) { throw "Launcher: engine 'PIM-Assignment-Wizard.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
    ...
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'PIM-Assignment-Wizard (community cloud) completed'
}
catch {
