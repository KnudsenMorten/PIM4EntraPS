...
<#
.SYNOPSIS
    Community VM launcher for PIM4EntraPS\PIM-extra. Invokes the engine
...
.DESCRIPTION
    Runs PIM4EntraPS\PIM-extra on a Windows box in the customer's own tenant.
...
$engine = Join-Path $solutionRoot 'engine\PIM-extra\PIM-extra.ps1'
if (-not (Test-Path -LiteralPath $engine)) {
    throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\PIM-extra\PIM-extra.ps1."
}
& $engine
