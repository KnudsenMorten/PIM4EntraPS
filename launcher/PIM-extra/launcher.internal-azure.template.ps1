...
<#
.SYNOPSIS
    Internal cloud-native launcher for PIM4EntraPS\PIM-extra.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'PIM-extra (internal cloud) starting'

try {
    $engine = Join-Path $InstallPath 'SOLUTIONS\PIM4EntraPS\CUSTOMSCRIPTS\PIM-extra.ps1'
    if (-not (Test-Path -LiteralPath $engine)) { throw "Cloud launcher: engine script not found at $engine." }
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'PIM-extra (internal cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "PIM-extra FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
