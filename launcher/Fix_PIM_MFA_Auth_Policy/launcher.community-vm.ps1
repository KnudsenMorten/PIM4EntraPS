...
<#
.SYNOPSIS
    Community VM launcher for PIM4EntraPS\Fix_PIM_MFA_Auth_Policy. Invokes the engine
...
.DESCRIPTION
    Runs PIM4EntraPS\Fix_PIM_MFA_Auth_Policy on a Windows box in the customer's own tenant.
...
.NOTES
    ...
    Engine         : Fix_PIM_MFA_Auth_Policy
...
$engine = Join-Path $solutionRoot 'engine\Fix_PIM_MFA_Auth_Policy\Fix_PIM_MFA_Auth_Policy.ps1'
if (-not (Test-Path -LiteralPath $engine)) {
    throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\Fix_PIM_MFA_Auth_Policy\Fix_PIM_MFA_Auth_Policy.ps1."
}
& $engine
