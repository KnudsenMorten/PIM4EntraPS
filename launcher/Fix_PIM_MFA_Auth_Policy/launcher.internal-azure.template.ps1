...
<#
.SYNOPSIS
    Internal cloud-native launcher for PIM4EntraPS\Fix_PIM_MFA_Auth_Policy.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'Fix_PIM_MFA_Auth_Policy (internal cloud) starting'

try {
    $engine = Join-Path $InstallPath 'SOLUTIONS\PIM4EntraPS\CUSTOMSCRIPTS\Fix_PIM_MFA_Auth_Policy.ps1'
    if (-not (Test-Path -LiteralPath $engine)) { throw "Cloud launcher: engine script not found at $engine." }
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'Fix_PIM_MFA_Auth_Policy (internal cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "Fix_PIM_MFA_Auth_Policy FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
