...
<#
.SYNOPSIS
    Community VM launcher for PIM4EntraPS\PIM-Assignment-Exporter-CSV-Only. Invokes the engine
...
.DESCRIPTION
    Runs PIM4EntraPS\PIM-Assignment-Exporter-CSV-Only on a Windows box in the customer's own tenant.
...
.NOTES
    ...
    Engine         : PIM-Assignment-Exporter-CSV-Only
...
if (-not (Test-Path -LiteralPath $engine)) {
    throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\PIM-Assignment-Exporter-CSV-Only\PIM-Assignment-Exporter-CSV-Only.ps1."
}
& $engine
