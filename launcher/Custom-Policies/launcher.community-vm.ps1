...
<#
.SYNOPSIS
    Community VM launcher for PIM4EntraPS\Custom-Policies. Invokes the engine
...
.DESCRIPTION
    Runs PIM4EntraPS\Custom-Policies on a Windows box in the customer's own tenant.
...
$engine = Join-Path $solutionRoot 'engine\Custom-Policies\Custom-Policies.ps1'
if (-not (Test-Path -LiteralPath $engine)) {
    throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\Custom-Policies\Custom-Policies.ps1."
}
& $engine
