...
<#
.SYNOPSIS
    Internal cloud-native launcher for PIM4EntraPS\Custom-Policies.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'Custom-Policies (internal cloud) starting'

try {
    $engine = Join-Path $InstallPath 'SOLUTIONS\PIM4EntraPS\CUSTOMSCRIPTS\Custom-Policies.ps1'
    if (-not (Test-Path -LiteralPath $engine)) { throw "Cloud launcher: engine script not found at $engine." }
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'Custom-Policies (internal cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "Custom-Policies FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
