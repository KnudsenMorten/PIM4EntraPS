...
<#
.SYNOPSIS
    Community cloud-native launcher for PIM4EntraPS\GetNumberOfROles.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'GetNumberOfROles (community cloud) starting'
...
if (-not $engine) { throw "Launcher: engine 'GetNumberOfROles.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
    ...
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'GetNumberOfROles (community cloud) completed'
}
catch {
