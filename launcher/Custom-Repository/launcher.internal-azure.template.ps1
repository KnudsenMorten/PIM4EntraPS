...
<#
.SYNOPSIS
    Internal cloud-native launcher for PIM4EntraPS\Custom-Repository.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'Custom-Repository (internal cloud) starting'

try {
    $engine = Join-Path $InstallPath 'SOLUTIONS\PIM4EntraPS\CUSTOMSCRIPTS\Custom-Repository.ps1'
    if (-not (Test-Path -LiteralPath $engine)) { throw "Cloud launcher: engine script not found at $engine." }
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'Custom-Repository (internal cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "Custom-Repository FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
