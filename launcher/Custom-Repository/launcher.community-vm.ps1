...
<#
.SYNOPSIS
    Community VM launcher for PIM4EntraPS\Custom-Repository. Invokes the engine
...
.DESCRIPTION
    Runs PIM4EntraPS\Custom-Repository on a Windows box in the customer's own tenant.
...
$engine = Join-Path $solutionRoot 'engine\Custom-Repository\Custom-Repository.ps1'
if (-not (Test-Path -LiteralPath $engine)) {
    throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\Custom-Repository\Custom-Repository.ps1."
}
& $engine
