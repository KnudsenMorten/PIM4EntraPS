...
<#
.SYNOPSIS
    Community VM launcher for PIM4EntraPS\Check-PIM-Groups-IsRoleAssignable. Invokes the engine
...
.DESCRIPTION
    Runs PIM4EntraPS\Check-PIM-Groups-IsRoleAssignable on a Windows box in the customer's own tenant.
...
.NOTES
    ...
    Engine         : Check-PIM-Groups-IsRoleAssignable
...
if (-not (Test-Path -LiteralPath $engine)) {
    throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\Check-PIM-Groups-IsRoleAssignable\Check-PIM-Groups-IsRoleAssignable.ps1."
}
& $engine
