...
<#
.SYNOPSIS
    Community cloud-native launcher for PIM4EntraPS\Check-PIM-Groups-IsRoleAssignable.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'Check-PIM-Groups-IsRoleAssignable (community cloud) starting'
...
if (-not $engine) { throw "Launcher: engine 'Check-PIM-Groups-IsRoleAssignable.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
    ...
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'Check-PIM-Groups-IsRoleAssignable (community cloud) completed'
}
catch {
