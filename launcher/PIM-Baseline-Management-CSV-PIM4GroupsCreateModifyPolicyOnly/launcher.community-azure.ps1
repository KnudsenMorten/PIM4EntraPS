...
<#
.SYNOPSIS
    Community cloud-native launcher for PIM4EntraPS\PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly (community cloud) starting'
...
$engine = Join-Path $solutionRoot 'engine\PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly\PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly.ps1'
if (-not (Test-Path -LiteralPath $engine)) { throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly\PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly.ps1." }
& $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly (community cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
