...
<#
.SYNOPSIS
    Internal cloud-native launcher for PIM4EntraPS\PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly (internal cloud) starting'
...
if (-not $engine) { throw "Launcher: engine 'PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
    ...
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly (internal cloud) completed'
}
catch {
