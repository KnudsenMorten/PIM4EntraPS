...
<#
.SYNOPSIS
    Community VM launcher for PIM4EntraPS\SQL-Connect. Invokes the engine
...
.DESCRIPTION
    Runs PIM4EntraPS\SQL-Connect on a Windows box in the customer's own tenant.
...
$engine = Join-Path $solutionRoot 'engine\SQL-Connect\SQL-Connect.ps1'
if (-not (Test-Path -LiteralPath $engine)) {
    throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\SQL-Connect\SQL-Connect.ps1."
}
& $engine
