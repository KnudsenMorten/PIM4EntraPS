...
<#
.SYNOPSIS
    Internal cloud-native launcher for PIM4EntraPS\SQL-Connect.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'SQL-Connect (internal cloud) starting'
...
try {
    ...
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'SQL-Connect (internal cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "SQL-Connect FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
