...
<#
.SYNOPSIS
    Community cloud-native launcher for PIM4EntraPS\PIM-Baseline-Management-SQL.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'PIM-Baseline-Management-SQL (community cloud) starting'
...
if (-not $engine) { throw "Launcher: engine 'PIM-Baseline-Management-SQL.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
    ...
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'PIM-Baseline-Management-SQL (community cloud) completed'
}
catch {
