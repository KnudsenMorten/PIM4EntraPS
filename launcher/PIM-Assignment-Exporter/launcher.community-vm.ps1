...
<#
.SYNOPSIS
    Community launcher for PIM4EntraPS\PIM-Assignment-Exporter (user VM / box).
.DESCRIPTION
...
if (-not (Test-Path -LiteralPath $engine)) { throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\PIM-Assignment-Exporter\PIM-Assignment-Exporter.ps1." }
& $engine
