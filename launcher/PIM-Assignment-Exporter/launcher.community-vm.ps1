...
<#
.SYNOPSIS
    Community VM launcher for PIM4EntraPS\PIM-Assignment-Exporter. Invokes the engine
...
.DESCRIPTION
    Runs PIM4EntraPS\PIM-Assignment-Exporter on a Windows box in the customer's own tenant.
...
.NOTES
    ...
    Engine         : PIM-Assignment-Exporter
...
$engine = Join-Path $solutionRoot 'engine\PIM-Assignment-Exporter\PIM-Assignment-Exporter.ps1'
if (-not (Test-Path -LiteralPath $engine)) {
    throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\PIM-Assignment-Exporter\PIM-Assignment-Exporter.ps1."
}
& $engine
