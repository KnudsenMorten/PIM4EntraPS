...
<#
.SYNOPSIS
    Community VM launcher for PIM4EntraPS\PIM-Baseline-Management-CSV-EntraIDRolesOnly. Invokes the engine
...
.DESCRIPTION
    Runs PIM4EntraPS\PIM-Baseline-Management-CSV-EntraIDRolesOnly on a Windows box in the customer's own tenant.
...
.NOTES
    ...
    Engine         : PIM-Baseline-Management-CSV-EntraIDRolesOnly
...
if (-not (Test-Path -LiteralPath $engine)) {
    throw "Launcher: engine script not found at $engine. Expected <solroot>\engine\PIM-Baseline-Management-CSV-EntraIDRolesOnly\PIM-Baseline-Management-CSV-EntraIDRolesOnly.ps1."
}
& $engine
