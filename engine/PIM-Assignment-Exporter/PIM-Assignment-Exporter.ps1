<#
.SYNOPSIS
    PIM-Assignment-Exporter - engine script in the PIM4EntraPS solution.

.NOTES
    Solution       : PIM4EntraPS
    File           : PIM-Assignment-Exporter.ps1
...
##########################################################################################################################
    ...
    $FileOutput = $FileOutputPath + "\" + "Security-Delegations.xlsx"
    ...
    Remove-Item $FileOutput -ErrorAction SilentlyContinue
...
#############################################
# Azure Resource Assignments (permanent)
#############################################

    $TableName      = "Azure_Resource_Assignments_Perm"
    $TableSelection = $PermAzResourceRoleAssignmentsArray
    If ($TableSelection)
        {
            Write-Output "Exporting $($TableName) .... Please Wait !"
            $TableSelection | Export-Excel -Path $FileOutput -WorksheetName $TableName -AutoFilter -AutoSize -BoldTopRow -tablename $TableName -tablestyle Medium13
...
#############################################
        ...
        $Pivottable.PT_Entra_ID_Roles  = @{ SourceWorkSheet = "Entra_Id_Role_Assignments"
                                            PivotRows       = @('RoleDisplayName','AssignmentType','PrincipalDisplayName')
                                            PivotData       = @{"CreatedDateTime"="Count"}
                                          }

        $Pivottable.PT_AzRes_PIM_Roles  = @{ SourceWorkSheet = "Azure_Resource_Assignments_PIM"
                                               PivotRows       = @('ResourceScopeDisplayName','RoleDisplayName','AssignmentType','PrincipalType','PrincipalDisplayName')
                                               PivotData       = @{"CreatedDateTime"="Count"}
                                             }

        $Pivottable.PT_PIM_Groups  = @{ SourceWorkSheet = "PIM_for_Groups"
                                        PivotRows       = @('GroupDisplayName','AssignmentType','PrincipalDisplayName')
                                        PivotData       = @{"CreatedDateTime"="Count"}
                                      }

        $Pivottable.PT_AzRes_Perm_Roles_View1  = @{ SourceWorkSheet = "Azure_Resource_Assignments_Perm"
                                                        PivotRows       = @('Scope','RoleName','PrincipalType','PrincipalName','PrincipalUPN')
                                                        PivotData       = @{"PrincipalId"="Count"}
                                                    }

        $Pivottable.PT_AzRes_Perm_Roles_View2  = @{ SourceWorkSheet = "Azure_Resource_Assignments_Perm"
                                                          PivotRows       = @('PrincipalType','PrincipalName','PrincipalUPN','RoleName','Scope')
                                                          PivotData       = @{"PrincipalId"="Count"}
                                                       }
    ...
    write-host "Export can be found here -> $($FileOutput)"
    ...
    If (!($global:Mail_PIM_Exporter_Delegations_Permissions_SendMail))
        {
            Write-host "Missing Default variables to send email"
        }
    Else
        {
            $TitleAlert       = "Permission Exporter - PIM & Permanent" 
            $BodyAlert        = "<font color=red>Export of Entra ID Roles, PIM for Groups and Azure Resource Permissions</font><br><br>"
            $BodyAlert       += "Attached you will find the current permissions/delegations<br><br>"
            #----------------------------------------------------------------------------------------------------------------------------------------------------------
            $global:Attachments     = @($FileOutput)
            $global:Channel         = 'Security'
            $MessageClassification  = 'Alert'            # Alert | Investigations | ProductChanges | ProductInfo | AutomationInfo | Info
            $AlertImpactUrgency     = 'P2'               # Alert-only: P1 = Critical Business (urgent for business, work impact) | P2 = High (department/location impact, work impact) | P3 = Moderate (Impact:Number of Users) | P4 = Low (Impact:Single user, Urgentcy:Low)

            If ($global:Mail_SendAnonymous -eq $true)
                {
                    If ($global:Mail_PIM_Exporter_Delegations_Permissions_SendMail -eq $true)  # Send mail
                        {
                            $global:To = $global:Mail_PIM_Exporter_Delegations_Permissions_To
