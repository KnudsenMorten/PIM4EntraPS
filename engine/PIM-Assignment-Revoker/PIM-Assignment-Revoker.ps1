<#
.SYNOPSIS
    PIM-Assignment-Revoker - engine script in the PIM4EntraPS solution.

.NOTES
    Solution       : PIM4EntraPS
    File           : PIM-Assignment-Revoker.ps1
...
While ($Step1_Result.Type -ne "Exit")
{
    ...
    #-----------------------------------------------------------------------------------------------
        ...
        $Step1_Choices = [PSCustomObject]@(
                                            [PSCustomObject]@{
                                                Type = "EntraID_Role_Group_PIM"
                                                Description = "Entra ID Role Assignment for Group or Admin Account, PIM-assigned"
                                                },
                                            [PSCustomObject]@{
                                                Type = "Azure_Resource_PIM"
                                                Description = "Azure Resource, PIM-assigned"
                                                },
                                            [PSCustomObject]@{
                                                Type = "Azure_Resource_Perm"
                                                Description = "Azure Resource, Permanent RBAC-assigned"
                                                },
                                            [PSCustomObject]@{
                                                Type = "PIM_Groups"
                                            ...
                                            [PSCustomObject]@{
                                                Type = "Rebuild"
                                                Description = "Get updated data from Entra, Azure & PIM (flush data in memory)"
                                                },
                                            [PSCustomObject]@{
                                                Type = "EXIT"
        ...
        $Step1_Result  = $Step1_Choices | Select-Object Description, Type | Out-GridView -Title "PIM Assignment Revoker | Step 1/2: Choose TYPE of Assignment or Exit" -PassThru
    ...
    If ($Step1_Result.Type -eq "EntraID_Role_Group_PIM")
        {
            # Scoping
            ...
            ForEach ($Entry in $DeleteAssignments)
                {
                    ...
                    write-host ""
                    Write-host "Removing role $($Entry.RoleDisplayName) for $($Entry.PrincipalDisplayName) in Entra ID Roles"
                    write-host ""
    ...
    #---------------------------------------------------------------------------------------------------------------------
    ElseIf ($Step1_Result.Type -eq "Azure_Resource_PIM")
        {
            ...
            $PIMAssignments = $Provisioned_PIMAzResourceRoleAssignmentsArray | 
                                                          Select-Object AssignmentType, `
            ...
            ForEach ($Entry in $DeleteAssignments)
                {
                    ...
                    ElseIf ( ($Entry.AssignmentType -eq "Assigned") -or ($Entry.AssignmentType -eq "Active") )
                        {
                    ...
                    write-host ""
                    Write-host "Removing role $($Entry.RoleDisplayName) for $($Entry.PrincipalDisplayName) on"
                    write-host $Entry.ResourceScope
                    write-host ""
    ...
    #---------------------------------------------------------------------------------------------------------------------
    ElseIf ($Step1_Result.Type -eq "Azure_Resource_Perm")
        {
            $Assignments = $PermAzResourceRoleAssignmentsArray | `
                                                  Select-Object Scope, `
                                                                RoleName, `
                                                                PrincipalType, `
                                                                PrincipalName, `
                                                                ...
                                                                PrincipalId, `
                                                                RoleId |
                                                Sort-Object -Property Scope, RoleName

            $DeleteAssignments = $Assignments | Out-GridView -Title 'Select Assignments to Delete' -PassThru

            ForEach ($Entry in $DeleteAssignments)
                {
                    write-host ""
                    Write-host "Removing role $($Entry.RoleName) for $($Entry.PrincipalName) on"
                    write-host $Entry.Scope
                    write-host ""
                    $result = Remove-AzRoleAssignment -ObjectId $Entry.PrincipalId `
                                                      -RoleDefinitionName $Entry.RoleName `
                                                      -Scope $Entry.Scope
                    $result | fl
                }
        }
            ...
            ForEach ($Entry in $DeleteAssignments)
                {
                    ...
                    write-host ""
                    Write-host "Removing assignment for $($Entry.PrincipalDisplayName) to group $($Entry.GroupDisplayName)"
                    write-host $Entry.ResourceScope
                    write-host ""
    ...
    #---------------------------------------------------------------------------------------------------------------------
    ElseIf ($Step1_Result.Type -eq "Rebuild")
        {
            BuildData
        }
    #---------------------------------------------------------------------------------------------------------------------
