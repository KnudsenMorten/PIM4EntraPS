<#
.SYNOPSIS
    PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly - engine script in the PIM4EntraPS solution.

.NOTES
    Solution       : PIM4EntraPS
    File           : PIM-Baseline-Management-CSV-PIM4GroupsCreateModifyPolicyOnly.ps1
        ...
        ForEach ($Entry in $Global:PAG_Groups_Definitions)
            {
                ...
                # Cannot convert value to type System.String.' (-ErrorAction does
                # NOT help: transformation failures happen at parameter binding,
                # before error-action preferences apply.)
...
######################################################################################################
    ...
    $PIM_Policy_Groups_Target = $PIM_Policy_Groups_Target | where-Object { ($_.DisplayName -notlike "PIM-AD-SRV*") } | Sort-Object -Property DisplayName
